import ZigzagSeriesModel from '../models/zigzagSeriesfolder/zigzagSeriesSchema.js';
...
// Create ZigzagSeries Model
export const createZigzagSeriesModel = async (req, res) => {
    try {
        let newModelData;
        ...
        if (req.is('multipart/form-data')) {
            newModelData = {
                ...
                zigzagItem1: req.body.zigzagItem1,
                zigzagItem2: req.body.zigzagItem2,
        ...
        const newModel = new ZigzagSeriesModel(newModelData);
        await newModel.save();
...
// Get all ZigzagSeries Models
export const getZigzagSeriesModels = async (req, res) => {
    try {
        const models = await ZigzagSeriesModel.find();
...
// Get ZigzagSeries Model by ID
export const getZigzagSeriesModelById = async (req, res) => {
    try {
        const model = await ZigzagSeriesModel.findById(req.params.id);
        if (!model) {
...
// Update ZigzagSeries Model
export const updateZigzagSeriesModel = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const model = await ZigzagSeriesModel.findByIdAndUpdate(id, updatedData, { new: true });
        if (!model) {
            return res.status(404).json({ message: 'Model not found' });
        }
...
// Delete ZigzagSeries Model
export const deleteZigzagSeriesModel = async (req, res) => {
    try {
        const model = await ZigzagSeriesModel.findByIdAndDelete(req.params.id);
        if (!model) {
            return res.status(404).json({ message: 'Model not found' });
        }
...
// Get ZigzagSeries Model Schema Fields
export const getZigzagSeriesModelSchemaFields = async (req, res) => {
    try {
        const schemaFields = Object.keys(ZigzagSeriesModel.schema.paths).reduce((fields, path) => {
            fields[path] = ZigzagSeriesModel.schema.paths[path].instance;
            return fields;
        }, {});
